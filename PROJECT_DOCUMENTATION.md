...
**Version:** 7.0.0 | **Last Updated:** March 1, 2026 | **Status:** Production
...
3. Run a unified Gemini AI analysis in a single API call (V7 ARCH-01)
...
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  POST /analyze/V7/unified  (ARCH-01 — Primary)           │   │
│  │  POST /analyze/V7/stream   (SSE Streaming fallback)      │   │
│  │  POST /analyze/V7          (Regular fallback)            │   │
...
7. Optional actions:
   ...
   └─ 📊 Bulk analysis (V7.13)     [Paste multiple URLs → batch results]
...
```
REQUEST ARRIVES: POST /analyze/V7/unified
...
Gemini returns single JSON with:
  ...
  ├─ evidence_spans[] {text, category, confidence}   ← V7.3
  └─ risk_assessment {score, level, warnings[]}

STEP 3 — POST-PROCESSING & SCORING (~0.5 seconds)
──────────────────────────────────────────────────
  ├─ RiskScorerV7: Weighted formula combining all signals
  ├─ ScoreReranker: Apply user correction adjustments (V7.9)
  ├─ Blocklist: Check domain against community reports
  ├─ Incognito flag: Check metadata for incognito pattern (V7.6)
...
FINAL RESPONSE (version: "7.0"):
{
  article_summary, sentiment_v7, toxicity_v7, fact_check_v7,
  risk_score_v7, comments_analysis, scam_analysis,
  evidence_spans,        ← V7.3 Explainable AI
  correction_history,    ← V7.9 Re-ranker
  incognito_flag,        ← V7.6
  blocklist_info, domain_feedback, learning_applied,
  url, version: "7.0", cache_stats
}
```
...
[FASTAPI /analyze/V7/unified]
     ...
     │  → ai_result{}                                        │
     │                                                       │
     │  score_reranker.apply(ai_result, url)  ← V7.9        │
...
| `SUBMIT_SCAM_REPORT`   | popup → background | `{url, scamType}`           | Scam URL report (V7.12)     |
| `SUBMIT_CORRECTION`    | popup → background | `{url, corrections{}}`      | Score correction (V7.9)     |
| `BULK_SCAN`            | popup → background | `{urls[]}`                  | Batch analysis (V7.13)      |
...
├── chrome_store_description_v7.txt # Chrome Web Store listing copy
│
├── src/
│   ├── __init__.py
│   ├── models/
│   │   ├── gemini_llm.py           # APIKeyRotator + GeminiAgent (30-key rotation)
│   │   ├── unified_analyzer.py     # ARCH-01: single-pass Gemini analysis (V7)
│   │   ├── sentiment_v7.py         # V7 sentiment: Gemini + PhoBERT + keyword fallback
│   │   ├── toxicity_v7.py          # V7 toxicity: 4-layer (regex→Gemini→Persp→Detoxify)
│   │   ├── fact_checker_v7.py      # V7 fact check: multi-source verification
│   │   ├── source_analyzer_v7.py   # Domain credibility: SSL/WHOIS/reputation
│   │   ├── risk_scorer_v7.py       # Weighted formula: fake(30%)+tox(25%)+sent(15%)+src(15%)+manip(15%)
│   │   ├── article_summarizer_v7.py# Gemini article summarization + URL cache
│   │   ├── reranker.py             # ScoreReranker: user correction re-ranking (V7.9)
...
│
└── extension/
    ├── manifest.json               # Chrome Manifest V3, version 7.0.0
    ...
    ├── domain_control.js           # Domain blacklist/whitelist module (V7.2)
...
#### `POST /analyze/V7/unified` — Primary (ARCH-01)
...
#### `POST /analyze/V7/stream` — SSE Streaming
...
The core innovation introduced in V7.0. Replaces 3–5 sequential Gemini calls with one structured prompt that returns all 6 analysis modules simultaneously.

**Token savings:** ~10,200 tokens (v3) → ~3,300 tokens (V7) — **68% reduction**
...
### 6.3 ScoreReranker — `src/models/reranker.py` (V7.9)
...
Feb 19     v5.9  ──── SSE streaming + YouTube/TikTok + overlay + notifications
           (shipped as v4.9 internally)
                │
Feb 23     V7.0  ──── ARCH-01 unified AI + explainable AI + incognito
                │         + score correction + scam detection + bulk analysis
                ▼
Mar  1     V7.0  ──── Production (current, main branch)
```
...
| Feature                                          | v2.0 |  v3.0  | v4.0 | v4.5 |     v5.0     |   v5.9/4.9   |     V7.0     |
| ------------------------------------------------ | :--: | :-----: | :--: | :--: | :----------: | :----------: | :----------: |
| **PLATFORM SUPPORT**                       |      |        |      |      |              |              |              |
| AI learning from corrections                     |  ❌  |   ❌   |  ❌  |  ❌  | **✅** |      ✅      |      ✅      |
| Real-time Chrome notifications                   |  ❌  |   ❌   |  ❌  |  ❌  |      ❌      | **✅** |      ✅      |
| **Incognito mode detection**               |  ❌  |   ❌   |  ❌  |  ❌  |      ❌      |      ❌      | **✅** |
| **Domain blacklist/whitelist (custom)**    |  ❌  |   ❌   |  ❌  |  ❌  |      ❌      |      ❌      | **✅** |
| **Scam URL 1-click reporting**             |  ❌  |   ❌   |  ❌  |  ❌  |      ❌      |      ❌      | **✅** |
| **Re-Ranker (AI self-corrects from user)** |  ❌  |   ❌   |  ❌  |  ❌  |      ❌      |      ❌      | **✅** |
| **SAFETY SYSTEMS**                         |      |        |      |      |              |              |              |
...
#### V7.0 — Architectural Overhaul (Current)

The biggest architectural change: **ARCH-01** replaced 3–5 sequential Gemini calls with a single unified call (68% fewer tokens, 60–75% faster). Added V7.3 explainable AI (text evidence spans), V7.6 incognito detection, V7.9 score correction with Re-Ranker model, V7.12 scam URL detection and reporting, V7.13 bulk analysis mode (multiple URLs at once), and V7.2 custom domain blacklist/whitelist. All version labels in UI and code unified to "V7".
...
| Task                       | Files to Edit                                                             |
| -------------------------- | ------------------------------------------------------------------------- |
| Change AI prompts          | `src/models/unified_analyzer.py`, `src/models/fact_checker_v7.py`     |
...
| Adjust risk weighting      | `src/models/risk_scorer_v7.py`                                          |
...
*Document generated: March 1, 2026 | VnContentGuard Pro V7.0.0*
