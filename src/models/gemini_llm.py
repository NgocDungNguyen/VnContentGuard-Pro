...
class APIKeyRotator:
    ...
    def __init__(self, api_keys: List[str]):
        ...
        self.cooldown_keys = {}  # key_index -> cooldown_until (datetime) for rate-limited keys
    ...
    def mark_key_rate_limited(self, retry_delay_seconds: float = 60.0):
        ...
        if count >= 5:
            print(f"🚫 API Key #{self.current_index + 1} exhausted (5+ consecutive 429s)")
        ...
        print(f"⏳ API Key #{self.current_index + 1} rate-limited, cooldown {retry_delay_seconds:.0f}s (attempt {count}/5)")
    ...
    def _wait_for_cooldown(self):
        ...
        if wait_seconds > 0 and wait_seconds <= 120:  # max 2 min wait
            print(f"⏳ All keys on cooldown. Waiting {wait_seconds:.0f}s for rate limit reset...")
    ...
    def _rotate_to_next_available(self) -> bool:
        ...
        for _ in range(len(self.api_keys)):
            ...
            if (self.current_index not in self.exhausted_keys and 
                self.current_index not in self.cooldown_keys):
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "available_count": len(self.api_keys) - len(self.exhausted_keys) - len(self.cooldown_keys),
