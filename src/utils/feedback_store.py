...
class FeedbackStore:
        ...
        with self._lock:
            try:
                ...
                entry = {
                    ...
                    "modules": modules or {},
                    "scan_snapshot": self._compress_snapshot(scan_results) if scan_results else {},
        ...
        try:
            ...
            if domain_feedbacks:
                ...
                if total > 0:
                    ...
                    lines.append(f"Trang {domain}: {total} lần quét, {accuracy:.0f}% chính xác.")
                    if accuracy < 60:
                        lines.append(f"⚠️ Độ chính xác thấp cho {domain} — hãy thận trọng hơn.")
            ...
            if total >= 10:
                ...
                if overall_accuracy < 70:
                    lines.append("⚠️ Hệ thống cần cải thiện — hãy phân tích kỹ hơn trước khi đưa kết luận.")
    ...
    def get_domain_feedback(self, url: str) -> dict:
        ...
        try:
            ...
            total = pos + neg
            corrections = [f.get("correction") for f in domain_fb if f.get("correction")]
    ...
    def _compress_snapshot(self, scan_results: dict) -> dict:
        ...
        try:
            return {
                ...
                "risk_score": scan_results.get("risk_score_v3", {}).get("risk_score", 0),
                "risk_level": scan_results.get("risk_score_v3", {}).get("risk_level", ""),
